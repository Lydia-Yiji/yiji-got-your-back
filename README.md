...
- 只要明显没动键鼠，或者娱乐太久时，才会轻轻提醒
- 即使任务还在计时，久未动键鼠提醒也会出现，帮你记得收尾
- 所有记录都只存在本机，不做手机同步
...
`Yiji Focus Float` 不需要额外权限也能正常使用：

- 双击开始 / 结束任务
- 单击查看 `Done Today` / `Done This Week`
- `娱乐 1 小时` 提醒

如果想要 `20 分钟没动键鼠` 的提醒效果，需要手动打开：

`系统设置 -> 隐私与安全性 -> Input Monitoring`

然后把 `Yiji Focus Float.app` 加进去并打开权限。

如果不给这个权限，也完全可以继续用，只是不会有那条 `喵，人在干什么？` 的久未活动提醒。
...
在 `Done Today` 里，会把成果文字直接贴进时间块；如果选了 `happy`，这一段右上角还会多一个小爱心。
